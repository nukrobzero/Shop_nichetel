...
$sql = "SELECT p_img FROM promotions WHERE p_id = '$p_id'"; 
$result = mysqli_query($conn,$sql);
if ($result->num_rows > 0) {
    $row = $result->fetch_assoc(); 
    $target_file2 = $target_dir.$row["p_img"];                
...
if ($uploadOk == 3) {
	...
	$sql = "UPDATE promotions SET p_img = '$file_name' WHERE p_id = '$p_id'"; 
	$result = mysqli_query($conn,$sql);
	...
	if ($result) {
		echo "
		<script> 
		window.location.href ='../admin/promotions_edit.php?p_id=$p_id';
		</script>
		";
	}
	else{
