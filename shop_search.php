    ...
    <!--                                 <?php
                                    ...
                                    <div class="dropdown">
                                        <button class="btn btn-light dropdown-toggle" type="button" data-toggle="dropdown" style="background-color: #D0D3D4;">Select Brand
                                          ...
                                          <ul class="dropdown-menu">
                                            ...
                                            <?php 
                                            $search1 =  htmlspecialchars( isset($_GET['search1'])?$_GET['search1']:'');
                                            if ($search1 !='') {
                                                $sql = "SELECT * FROM brand_products WHERE cg_id ='".$search1."'";
                                                $result = $conn->query($sql);
                                                while ($row = $result->fetch_assoc()) {
                                                    ?>
                    ...
                    <div class="row">
                        <div class="product__list another-product-style" style="text-align: center;">
                            ...
                            if ($search =='') {
                                $sql ="SELECT * FROM products INNER JOIN gallery_products ON products.pd_id = gallery_products.pd_id INNER JOIN brand_products ON products.brand_id = brand_products.brand_id INNER JOIN category ON brand_products.cg_id = category.cg_id INNER JOIN promotions ON products.p_id = promotions.p_id WHERE brand_products.cg_id = '".$search1."' GROUP BY products.pd_id ORDER BY products.pd_id  DESC";
                            }else{
                                $sql ="SELECT * FROM products INNER JOIN gallery_products ON products.pd_id = gallery_products.pd_id INNER JOIN brand_products ON products.brand_id = brand_products.brand_id INNER JOIN category ON brand_products.cg_id = category.cg_id INNER JOIN promotions ON products.p_id = promotions.p_id WHERE brand_name LIKE '%".$search."%' or pd_name LIKE '%".$search."%'   GROUP BY products.pd_id ORDER BY products.pd_id  DESC";
                            }
                            $result = $conn->query($sql);
